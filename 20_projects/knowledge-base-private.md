...
- 2026-07-02 · devin · 年报 baseline 第三批检查收尾（同/通/铁字头 20 家，PR #193 待用户 review，不合并）：批次 c42ad3b2 已完整落库（17 新建 + 3 合并实体页，log #2181），三闸门通过（integrity 0 错误 / evidence source 0 缺 / PIT 本批 20 篇齐）。收尾处理：①PR #193 与 main 冲突——log.md 用「theirs+append」策略合并并清理 16 行历史遗留 diff3 标记（`|||||||`/`=======`，曾被 pre-commit check-merge-conflict 拦截）；②entity_exposures/concept_graph 无 merge driver 覆盖，写一次性脚本做 3-way 语义并集（以 theirs 为骨架、追加 ours 相对 base 的新增 entity/concept/company 条目，冲突 0）；③log id 三度撞号：main 上晚间卖方研报最终占用 #2178，年报天/腾批次让位改 #2182（log.md + 40 个 entity/source 文件同步 sed）；④repair-meta 重建条数统计。遗留：同兴科技已有 F10 baseline，writer Rule 6 不覆盖旧画像区（关键数据仍"待补充"），年报数据只进 relations/证据区，如需以年报覆盖 F10 需人工决定。
