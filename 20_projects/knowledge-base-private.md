...
| RAG 实际入索引文件 manifest freshness | devin | doing | PR #269 open、CI 全绿；待 a77 修复后重建 110,065 chunks 并发布新 stable release |
...
- 2026-07-12 · devin · RAG freshness 修复已推送 [PR #269](https://github.com/linxiaoqi5111-del/knowledge-base-private/pull/269)，尚未合并：freshness 从只覆盖 `wiki/` 的 Git tree 改为对实际选择并入索引的 path + bytes SHA-256 计算 deterministic `manifest:v1:`，选择参数包含 `include_raw/max_files`；legacy `git-tree:` / `snapshot:` 一律返回 `unknown`。raw-only 新增/删除/修改/改名均有回归测试；manifest 路径改为与 `chunk_file()` 一致的 lexical repo path，避免外部 symlink 经 `.resolve()` 越出 repo，同时仍能检测目标 bytes 变化。另补 full-build 优化：当所有 chunk vectors 可复用时不加载约 2GB 的 BGE 模型。已从旧 release 资产复用 dense vectors 完成本地真实重建：chunks/dense/BM25 均 110,065，dense=`110065×1024`，`include_raw=true`、`max_files=0`、`progress=110065/110065`、`source_revision=manifest:v1:...`；发布脚本 stable dry-run 通过。验证：RAG 82 passed、pre-commit、GitHub `tests-and-gate` 全绿。**未完成**：`main` 尚不理解新 manifest，须用户 review/merge PR 后再用现有脚本发布 non-prerelease stable release，不能提升旧 prerelease或提前发布。
